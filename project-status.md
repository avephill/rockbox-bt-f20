...
| ~~C13b~~ | ~~Adaptive-bitrate refinement after the verified walk.~~ **Done 2026-07-24** — upshift **probe backoff**: an upshift knocked back down within 2 min doubles the next clean-period requirement (60 s → 2/4/8 min cap, `AAC probe fail, next in Ns` in the log; reset by a fresh distress episode or a new negotiation), ending the 60 s up/down cycle in sustained marginal conditions. Link-log ring grown 256 → 2048 entries (walk sessions were dropping up to ~1700 lines between pause-flushes), on-card cap 256 KB → 1 MB. Downshift threshold deliberately left at 200 ms — healthy indoor sessions routinely show 60–170 ms gaps, so a lower trigger would false-positive indoors. |
| ~~next~~ | ~~Outdoor walk verification.~~ **Passed 2026-07-23** (front-left pants pocket, BFP, user reports "definitely better outside"). Log shows the full C13 loop: `128000 -> 96000 (gaps)` at rssi ≈ −25/lq ≈ 85, second step `96000 -> 64000` 7 s later, then clean 87 pkt/2s windows at ~190 B; a `96000 -> 128000 (clean)` recovery earlier in the walk. **Worst stall all session: 300 ms** (pre-C13 walk: 21 s / 7.9 s / 1.7 s). Stall watchdog never fired — nothing froze. Zero MTU drops. RSSI ranged −33 (pocket) to +5 (out of pocket), TX pinned 12/12 throughout. Thresholds left as-is; ring overflow (`N dropped`) still swallows mid-session lines between pause-flushes — bump the ring only if a future diagnosis needs the missing stretch. |
